...
impl Field57A {
    ...
    pub fn description(&self) -> String {
        match &self.account_number {
            Some(account) => format!("Account With Institution: {} ({})", self.bic, account),
            None => format!("Account With Institution: {}", self.bic),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_field57a_creation() {
        let field = Field57A::new(None, None, "DEUTDEFF").unwrap();
        assert_eq!(field.bic(), "DEUTDEFF");
        assert!(field.account_number().is_none());
    }

    #[test]
    fn test_field57a_with_account() {
        let field = Field57A::new(None, Some("1234567890".to_string()), "DEUTDEFF500").unwrap();
    ...
    #[test]
    fn test_field57a_parse() {
        let field = Field57A::parse("CHASUS33").unwrap();
        assert_eq!(field.bic(), "CHASUS33");
    }

    #[test]
    fn test_field57a_to_swift_string() {
        let field = Field57A::new(None, None, "DEUTDEFF").unwrap();
        assert_eq!(field.to_swift_string(), ":57A:DEUTDEFF");
    }

    #[test]
    fn test_field57a_validation() {
        let field = Field57A::new(None, Some("1234567890".to_string()), "DEUTDEFF").unwrap();
