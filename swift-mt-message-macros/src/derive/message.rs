    ...
    let validation_impl = if let Some(rules_const) = validation_rules {
        ...
        quote! {
            impl #name {
                ...
                pub fn validate_business_rules(&self) -> crate::ValidationResult {
                    // Parse the validation rules JSON
                    let rules_json: serde_json::Value = match serde_json::from_str(#rules_ident) {
                        Ok(json) => json,
                        Err(e) => {
                            return crate::ValidationResult::with_error(
                                crate::errors::ValidationError::BusinessRuleValidation {
                    ...
                    // Serialize the message to JSON for data context
                    let message_data = match serde_json::to_value(self) {
                        Ok(data) => data,
                        Err(e) => {
                            return crate::ValidationResult::with_error(
                                crate::errors::ValidationError::BusinessRuleValidation {
                                    rule_name: "DATA_SERIALIZATION".to_string(),
                                    message: format!("Failed to serialize message data: {}", e),
                                }
                            );
                        }
                    };
                    ...
                    // Add message fields
                    if let serde_json::Value::Object(msg_obj) = message_data {
                    ...
                    // Add helper context for common patterns
                    data_context.insert("message_context".to_string(), serde_json::json!({
                        "message_type": #message_type,
                        "sender_country": "US", // TODO: Extract from BIC
                        "receiver_country": "DE" // TODO: Extract from BIC
                    }));

                    let context_value = serde_json::Value::Object(data_context);
                            ...
                            match dl.evaluate_json(condition, &context_value, None) {
                                Ok(result) => {
                                    match result.as_bool() {
                                        ...
                                        Some(false) => {
                                            // Rule failed
                                            errors.push(crate::errors::ValidationError::BusinessRuleValidation {
