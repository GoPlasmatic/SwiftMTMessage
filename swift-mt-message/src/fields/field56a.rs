...
impl Field56A {
    ...
    pub fn description(&self) -> String {
        match &self.account_number {
            Some(account) => format!("Intermediary Institution: {} ({})", self.bic, account),
            None => format!("Intermediary Institution: {}", self.bic),
        }
    }
}
...
impl SwiftField for Field56A {
    fn parse(value: &str) -> Result<Self, crate::ParseError> {
        let content = if value.starts_with(":56A:") {
            &value[5..]
        } else if value.starts_with("56A:") {
            &value[4..]
        } else {
            value
        };
