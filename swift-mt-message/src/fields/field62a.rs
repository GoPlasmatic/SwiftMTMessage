...
///
/// ## Overview
/// Field 62A represents the closing balance of an account for statement reporting.
...
impl Field62A {
    /// Create a new Field62A with validation
    pub fn new(
...
impl SwiftField for Field62A {
    fn parse(content: &str) -> Result<Self, ParseError> {
        let balance = GenericBalanceField::parse_with_tag(content, "62A")?;
        Ok(Field62A { balance })
    }

    fn to_swift_string(&self) -> String {
        self.balance.to_swift_string_with_tag("62A")
    }
...
impl std::fmt::Display for Field62A {
