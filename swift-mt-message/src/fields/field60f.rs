...
/// # Field 60F - Opening Balance (Intermediate)
///
/// ## Overview
/// Field 60F represents the opening balance of an account for statement reporting.
...
/// - **Intermediate Balance**: Balance at a specific point in time
...
impl Field60F {
    /// Create a new Field60F with validation
    pub fn new(
...
impl SwiftField for Field60F {
    fn parse(content: &str) -> Result<Self, ParseError> {
        let balance = GenericBalanceField::parse_with_tag(content, "60F")?;
        Ok(Field60F { balance })
    }

    fn to_swift_string(&self) -> String {
        self.balance.to_swift_string_with_tag("60F")
    }
...
impl std::fmt::Display for Field60F {
